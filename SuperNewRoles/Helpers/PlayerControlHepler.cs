...
|| GameStartManagerUpdatePatch.VersionPlayers.ContainsKey(player);
        ...
        public static void RefreshRoleDescription(PlayerControl player)
        {
            ...
            foreach (IntroData roleInfo in infos)
            {
                ...
                if (player.IsLovers())
                {
                    task.Text += "\n" + ModHelpers.Cs(Lovers.color, ModTranslation.GetString("LoversName") + ": " + string.Format(ModTranslation.GetString("LoversIntro"), PlayerControl.LocalPlayer.GetOneSideLovers()?.Data?.PlayerName ?? ""));
                }
