        ...
        public static void ApplyChanges(ShipStatus __instance)
        {
            if (Data.IsMap(CustomMapNames.Agartha))
            {
                if (__instance.Type == ShipStatus.MapType.Hq)
                {
                    ...
                    Wall.gameObject.AddComponent<EdgeCollider2D>().points =
                           new Vector2[] {
                               new Vector2(-5f, 19.3f),new Vector2(-5f, 25.5f),new Vector2(0f, 25.5f),new Vector2(0f, 23.7f), new Vector2(-1f, 23.7f),new Vector2(-2f, 21.7f),new Vector2(-3f, 19.3f),new Vector2(-5f, 19.3f)
                           };
                    Wall.gameObject.AddComponent<EdgeCollider2D>().points =
                           new Vector2[] {
