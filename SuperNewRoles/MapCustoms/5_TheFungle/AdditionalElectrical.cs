...
public static class FungleAdditionalElectrical
{
    public static void CreateElectrical()
    {
        ...
        ShipStatus skeld = MapLoader.Skeld;

        if (airship == null || skeld == null)
            return;

        List<PlayerTask> Tasks = ShipStatus.Instance.SpecialTasks.ToList();
        Tasks.Add(airship.SpecialTasks.FirstOrDefault(x => x.TaskType == TaskTypes.FixLights));
        ShipStatus.Instance.SpecialTasks = new(Tasks.ToArray());

        Console console1 = GameObject.Instantiate(airship.transform.FindChild("Storage/task_lightssabotage (cargo)"), fungleShipStatus.transform).GetComponent<Console>();
        console1.transform.localPosition = new(-16.2f, 7.67f, 0);
        console1.ConsoleId = 0;

        Console console2 = GameObject.Instantiate(skeld.transform.FindChild("Electrical/Ground/electric_frontset/SwitchConsole"), fungleShipStatus.transform).GetComponent<Console>();
    ...
    class MapBehaviourAwakePatch
    {
        public static void Postfix(MapBehaviour __instance)
        {
            ...
            MapRoom mapRoom = GameObject.Instantiate(airship.MapPrefab.infectedOverlay.rooms.FirstOrDefault(x => x.room == SystemTypes.Electrical), __instance.infectedOverlay.transform);
