    ...
    class CoEnterVent
    {
        public static bool Prefix(PlayerPhysics __instance, int id)
        {
            ...
            } else if (!RoleClass.Egoist.UseVent && __instance.myPlayer.isRole(RoleId.Egoist))
            {
                ...
                return false;
            } else if (__instance.myPlayer.isRole(RoleId.RemoteSheriff) || __instance.myPlayer.isRole(RoleId.Sheriff) || __instance.myPlayer.isRole(RoleId.truelover) || __instance.myPlayer.isRole(RoleId.FalseCharges))
            {
                MessageWriter writer = AmongUsClient.Instance.StartRpcImmediately(__instance.NetId, (byte)RpcCalls.BootFromVent, SendOption.Reliable, -1);
                writer.WritePacked(127);
                AmongUsClient.Instance.FinishRpcImmediately(writer);
