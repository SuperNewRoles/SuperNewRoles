            ...
            foreach (PlayerControl p in PlayerControl.AllPlayerControls)
            {
                if (!p.Data.Disconnected && p.IsPlayer())
                {
                    ...
                    if (!p.IsMod() && p.isAlive())
                    {
                        ...
                        if (p.isRole(RoleId.Scavenger) && RoleClass.Scavenger.IsCheck)
                        {
                            foreach (PlayerControl p2 in PlayerControl.AllPlayerControls)
                            {
                                if (!p2.Data.Disconnected && !p2.isRole(RoleId.Jackal))
                                ...
                                if (!p2.Data.Disconnected && !p2.isRole(RoleId.Sheriff))
                                {
                                    p2.RpcSetNamePrivate(p2.getDefaultName(), p);
                                }
                                else if (!p2.Data.Disconnected && p2.isRole(RoleId.Sheriff))
                                {
                                    p2.RpcSetNamePrivate(ModHelpers.cs(RoleClass.Sheriff.color, p2.getDefaultName()), p);
                                }
                                if (!p2.Data.Disconnected && !p2.isImpostor())
                                {
                                    p2.RpcSetNamePrivate(p2.getDefaultName(), p);
