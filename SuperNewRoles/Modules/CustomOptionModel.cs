...
static class GameOptionsMenuUpdatePatch
{
    ...
    {
        return option.isHidden || (!option.isSHROn && ModeHandler.IsMode(ModeId.SuperHostRoles, false)) || ((option == CustomOptionHolder.LoversBreakerOption ||| option == CustomOptionHolder.JumboOption) && DateTime.UtcNow < new DateTime(2022,12,23,12,0,0));
    }
    public static void Postfix(GameOptionsMenu __instance)
    {
        var gameSettingMenu = UnityEngine.Object.FindObjectsOfType<GameSettingMenu>().FirstOrDefault();
