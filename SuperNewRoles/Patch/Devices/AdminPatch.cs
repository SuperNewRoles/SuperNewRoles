...
namespace SuperNewRoles.Patch
{
    ...
    public class AdminPatch
    {
        ...
        {
            adminTimer = MapOptions.MapOption.CanUseAdminTime.GetFloat();
        ...
        public static bool CanUseAdmin { get { return MapOptions.MapOption.RestrictAdmin.GetBool() || RestrictAdminTime > 0f; } }
        ...
        static void UseAdminTime()
        {
            ...
            {
                MessageWriter writer = AmongUsClient.Instance.StartRpcImmediately(PlayerControl.LocalPlayer.NetId, (byte)CustomRPC.UseAdminTime, SendOption.Reliable, -1);
            ...
            static bool Prefix(MapCountOverlay __instance)
            {
                if (MapOptions.MapOption.MapOptionSetting.GetBool() && MapOptions.MapOption.RestrictAdmin.GetBool() && Mode.ModeHandler.IsMode(Mode.ModeId.Default) && !PlayerControl.LocalPlayer.IsRole(RoleId.EvilHacker))
                {
                    ...
                    if (MapOptions.MapOption.RestrictAdmin.GetBool() && MapOptions.MapOption.MapOptionSetting.GetBool() && MapOptions.MapOption.RestrictAdmin.GetBool())
                    {
                        ...
                        var timeString = TimeSpan.FromSeconds(RestrictAdminTime).ToString(@"mm\:ss\.ff");
                    ...
                    for (int i = 0; i < __instance.CountAreas.Length; i++)
                    {
                        ...
                        if (!commsActive)
                        {
                            ...
                            if (plainShipRoom != null && plainShipRoom.roomArea)
                            {
                                ...
                                for (int j = 0; j < num; j++)
                                {
                                    ...
                                    if (!(collider2D.tag == "DeadBody"))
                                    {
                                        ...
                                        if (!component || component.Data == null || component.IsDead())
                                        {
                                            num2--;
                                        }
                                        else if (component?.MyRend()?.material != null)
                                        {
                                            var color = component.MyRend().material.GetColor("_BodyColor");
                            ...
                            {
                                Logger.Error($"Couldn't find counter for:{counterArea.RoomType}", "AdminPatch");
                            }
                        }
                        else
                        {
