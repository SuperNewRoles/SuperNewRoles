...
class AddChatPatch
{
    ...
    public static bool Prefix(PlayerControl sourcePlayer, string chatText)
    {
            ...
            )
        {
            ...
            Commands[0].Equals("/RuleSee", StringComparison.OrdinalIgnoreCase) ||
            Commands[0].Equals("/rule", StringComparison.OrdinalIgnoreCase))
        {
            SendCommand(sourcePlayer, "ルールを表示する前に、ルールに関する注意点です。\n以下の点を守ってください。\n・このモードについて、SNSに上げたり他の人に無断で上げたりしないこと\n上記のルールを守れる場合は、/crと打ち込んでください。");
            return false;
        }
        else if (
            Commands[0].Equals("/cr", StringComparison.OrdinalIgnoreCase))
        {
            string text = "このモードのルールについて紹介します。\n【パンツ大騒動】\n皆さんには、今からパンツを奪い合うデスゲームを行ってもらいます。\nルールは簡単、制限時間内にパンツを相手から奪い取り、履きましょう。\nパンツは、キルを行うことで相手から奪い取ることができます。\n制限時間内にパンツを履けなかった場合、死亡してしまいます。\nコレを繰り返し、最後に生き残った者のみがパンツを手にすることができます。\nそれでは、健闘を祈る。\n\nバグやこうした方がいいんじゃない?というのはどんどん言ってください\nルールがわかった人は\n/ok " + sourcePlayer.PlayerId.ToString() + "\nというコマンドを送信してください。";
            SendCommand(sourcePlayer, text);
            return false;
        }
        else if (
            Commands[0].Equals("/ok", StringComparison.OrdinalIgnoreCase) || Commands[0] == "OK" + sourcePlayer.PlayerId.ToString())
        {
            if (Commands.Length <= 1 && Commands[0] != "OK" + sourcePlayer.PlayerId.ToString())
            {
                SendCommand(sourcePlayer, "正常なコマンドを入力してください。\nルールは、/ruleと入力することで見えます。");
            }
            else if (Commands[0] == "OK" + sourcePlayer.PlayerId.ToString() || Commands[1] == sourcePlayer.PlayerId.ToString())
            {
                if (!RoleClass.RuleAgrees.Contains(sourcePlayer.Data.FriendCode))
                {
                    RoleClass.RuleAgrees.Add(sourcePlayer.Data.FriendCode);
                    SendCommand(sourcePlayer, "ルールに同意しました!");
                    int DOUIZUMI = 0;
                    int a = 0;
                    foreach (PlayerControl player in PlayerControl.AllPlayerControls)
                    {
                        if (RoleClass.RuleAgrees.Contains(player.Data.FriendCode))
                            DOUIZUMI++;
                        a++;
                    }
                    SendCommand(PlayerControl.LocalPlayer, sourcePlayer.Data.PlayerName + "が同意しました。\n現在の同意者数:(" + DOUIZUMI.ToString() + "/" + a.ToString() + ")");
                }
                else
                {
                    SendCommand(sourcePlayer, "もう同意済みです。");
                }
            }
            else
            {
            ...
            Commands[0].Equals("/OKPlayers", StringComparison.OrdinalIgnoreCase))
        {
            if (sourcePlayer.PlayerId == 0)
            {
                string text = "";
                int DOUIZUMI = 0;
                int a = 0;
                foreach (PlayerControl player in PlayerControl.AllPlayerControls)
                {
                    text += player.Data.PlayerName + ":" + (RoleClass.RuleAgrees.Contains(player.Data.FriendCode) ? "同意済み" : "未同意") + "\n";
                    if (RoleClass.RuleAgrees.Contains(player.Data.FriendCode))
                        DOUIZUMI++;
                    a++;
                }
                text += "同意済み:(" + DOUIZUMI.ToString() + "/" + a.ToString() + ")";
                SendCommand(sourcePlayer, text);
            }
            return false;
        }
        else if (
            Commands[0].Equals("/OKKicks", StringComparison.OrdinalIgnoreCase))
        {
            if (sourcePlayer.PlayerId == 0)
            {
                foreach (PlayerControl player in PlayerControl.AllPlayerControls)
                {
                    if (!RoleClass.RuleAgrees.Contains(player.Data.FriendCode) && player.GetClientId() != PlayerControl.LocalPlayer.GetClientId())
                        AmongUsClient.Instance.KickPlayer(player.GetClientId(), false);
                }
                SendCommand(sourcePlayer, "キックしました。");
            }
            return false;
        }
        else
