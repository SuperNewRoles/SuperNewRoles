...
public class CursedDressUpTask
{
    ...
    public static class NormalPlayerTaskPatch
    {
        ...
        public static void InitializePostfix(NormalPlayerTask __instance)
        {
            ...
            for (int i = 0; i < 3; i++) __instance.Data[i] = (byte)UnityEngine.Random.Range(0, 3);
    ...
    public static class ShipStatusPatch
    {
        ...
        {
            if (!Main.IsCursed) return;
            if (!MapNames.Airship.IsMap()) return;
        ...
        private static IEnumerator NotMoveUsePlatform(MovingPlatformBehaviour platform)
        {
            ...
            if (platform.Target != null) yield break;

            Vector3 targetPos = platform.IsLeft ? platform.RightPosition : platform.LeftPosition;
            ...
            if (Constants.ShouldPlaySfx())
            {
                ...
                SoundManager.Instance.PlayDynamicSound(
                    "PlatformMoving",
                    platform.MovingSound,
                    loop: true,
                    (DynamicSound.GetDynamicsFunction)platform.SoundDynamics,
                    SoundManager.instance.sfxMixer);
            }

            platform.IsLeft = !platform.IsLeft;
            yield return Effects.Slide2D(platform.transform, platform.transform.localPosition, targetPos, PlayerControl.LocalPlayer.MyPhysics.Speed);
