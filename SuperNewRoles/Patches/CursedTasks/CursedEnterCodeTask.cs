...
public class CursedEnterCodeTask
{
    ...
    public static string GetCode(out string card)
    {
        ...
        card = string.Empty;
        if (ModHelpers.GetRandomFloat(1f) < 0.8f)
        {
            int length = Random.Range(5, 21);
        ...
        else if (ModHelpers.GetRandomFloat(1f) < 0.5f)
        {
    ...
    public static class EnterCodeMinigamePatch
    {
        ...
        public static void BeginPostfix(EnterCodeMinigame __instance)
        {
            ...
            var background = AssetManager.GetAsset<Sprite>("EnterCode.png");
            if (background) __instance.transform.FindChild("Background").GetComponent<SpriteRenderer>().sprite = background;
