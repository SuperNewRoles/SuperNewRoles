            ...
            /*public static bool Prefix(GameStartManager __instance)
            {
                ...
                if (AmongUsClient.Instance.AmHost)
                {
                    if (NameIncludeMod && !NameIncludeSNR && !NameIncludeSHR)
                    {
                        ...
                        CachedPlayer.LocalPlayer.PlayerControl.RpcSendChat(string.Format("Modが名前に含まれている状態では公開部屋にすることはできません。"));
                        return false;
                    }
                    else if ((ModeHandler.IsMode(ModeId.SuperHostRoles, false) && NameIncludeSNR && !NameIncludeSHR) || (ModeHandler.IsMode(ModeId.SuperHostRoles, false) && NameIncludeMod && !NameIncludeSHR))
                    {
                        ...
                        CachedPlayer.LocalPlayer.PlayerControl.RpcSendChat(string.Format("SHRモードでSNRが名前に含まれている状態では公開部屋にすることはできません。"));
                        return false;
                    }
                    else if (!NameIncludeSNR && !NameIncludeSHR)
                    {
                        ...
                        __instance.MakePublicButton.color = Palette.DisabledClear;
                        __instance.privatePublicText.color = Palette.DisabledClear;
                        CachedPlayer.LocalPlayer.PlayerControl.RpcSendChat(string.Format("Mod関連のワードが名前に入っていないと公開部屋にすることはできません。特殊モードをご利用の場合は名前に「SHR」を入れてください。"));
                        return false;
                    }
                }
                return true;
