        ...
        public static bool Prefix(MeetingHud __instance)
        {
            try
            {
                ...
                else
                {
                    foreach (var ps in __instance.playerStates)
                    { //死んでいないプレイヤーが投票していない
                        if (!(ps.AmDead || ps.DidVote) && ModHelpers.PlayerById(ps.TargetPlayerId) != null && !ModHelpers.PlayerById(ps.TargetPlayerId).IsBot())
                            return false;
                    }
                }
                VoterState[] states;
    ...
    static class ExtendedMeetingHud
    {
        public static Dictionary<byte, int> CustomCalculateVotes(this MeetingHud __instance)
        {
            ...
            for (int i = 0; i < __instance.playerStates.Length; i++)
            {
                ...
                {
                    int VoteNum = 1;
                    if (CheckForEndVotingPatch.VoteCountDictionary.ContainsKey(ModHelpers.PlayerById(ps.TargetPlayerId).GetRole()))
                        VoteNum = CheckForEndVotingPatch.VoteCountDictionary[ModHelpers.PlayerById(ps.TargetPlayerId).GetRole()];
