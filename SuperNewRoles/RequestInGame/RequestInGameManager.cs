...
public class RequestInGameManager
{
    public class Thread
    {
        ...
        public Thread(string thread_id, string title, string created_at)
        {
    ...
    public static async Task<List<Thread>> GetThreads(bool unreadOnly = false)
    {
        using (HttpClient client = await CreateHttpClient())
        {
            ...
            (
                title: thread.GetProperty("data").GetProperty("title").GetString() ?? string.Empty,
    ...
    private static async Task<HttpClient> CreateHttpClient()
    {
        ...
        client.DefaultRequestHeaders.Authorization = new System.Net.Http.Headers.AuthenticationHeaderValue("Bearer", await GetOrCreateToken());
        return client;
    }
}