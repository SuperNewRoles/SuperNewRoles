            ...
            foreach (IntroDate roleInfo in IntroDate.IntroDatas)
            {
                ...
                if (PlayerControl.LocalPlayer.IsAlive()) button.GetComponent<PassiveButton>().OnClick.AddListener((System.Action)(() =>
                {
                    ...
                    else
                    {
                        ...
                        MessageWriter writer = AmongUsClient.Instance.StartRpcImmediately(PlayerControl.LocalPlayer.NetId, (byte)CustomRPC.CustomRPC.GuesserShoot, SendOption.Reliable, -1);
