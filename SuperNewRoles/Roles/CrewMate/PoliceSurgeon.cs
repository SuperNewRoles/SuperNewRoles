...
public static class PoliceSurgeon
{
    ...
    internal class PersonalInformationOfTheDead
    {
        ...
        internal static string GetPostMortemCertificate(PersonalInformationOfTheDead info) =>
            info.PostMortemCertificate.Replace(RoleData.ElapsedTurn, $"{ReportDeadBodyPatch.MeetingCount.allTurn - info.DeadTurn}").Replace(RoleData.PoliceSurgeonName, PlayerControl.LocalPlayer.name);
...
#pragma warning restore 8321
    ...
    private static void AddActualDeathTime(DeadTiming timingOfCall, PlayerControl playerWhoPlansToDie = null)
    {
        ...
        foreach (PlayerControl p in CachedPlayer.AllPlayers)
        {
            ...
            RoleData.PersonalInformationManager.Add(PersonalInformationOfTheDead.Create(p.PlayerId, ReportDeadBodyPatch.MeetingCount.allTurn, deadReason, DeadPlayer.ActualDeathTime[p.PlayerId].DeathTime, estimatedDeathTime));
            ...
            writer.Write((byte)ReportDeadBodyPatch.MeetingCount.allTurn);
            writer.EndRPC();
        }
    }
...
#pragma warning restore 8321
    ...
    private static void Event(MeetingHud __instance)
    {
        ...
        for (int i = 0; i < __instance.playerStates.Length; i++)
        {
            ...
            if (!CustomOptionData.IndicateTimeOfDeathInSubsequentTurn.GetBool() && player.PlayerId != CachedPlayer.LocalPlayer.PlayerId && personalInfo.Item1 && personalInfo.Item2.DeadTurn != ReportDeadBodyPatch.MeetingCount.allTurn) continue;
...
internal static class PostMortemCertificate_CreateAndGet
{
    ...
    {
        if (!RoleData.PostMortemCertificateFullText.ContainsKey(ReportDeadBodyPatch.MeetingCount.allTurn)) RoleData.PostMortemCertificateFullText.Add(ReportDeadBodyPatch.MeetingCount.allTurn, CreateBody());
        return RoleData.PostMortemCertificateFullText[ReportDeadBodyPatch.MeetingCount.allTurn].Replace(RoleData.PoliceSurgeonName, policeSurgeon.name);
    }
    ...
    internal static string CreateBody(PersonalInformationOfTheDead victimInfo = null)
    {
        ...
        if (victimInfo == null) // 特定のプレイヤー指定ではない時、全員の死体検案書を取得する。
        {
            foreach (var info in RoleData.PersonalInformationManager)
            {
                ...
                if (!CustomOptionData.IndicateTimeOfDeathInSubsequentTurn.GetBool() && info.DeadTurn != ReportDeadBodyPatch.MeetingCount.allTurn) continue;
                ...
                builder.Append(CreateContents(info).Replace(RoleData.ElapsedTurn, $"{ReportDeadBodyPatch.MeetingCount.allTurn - info.DeadTurn}"));
                builder.AppendLine(delimiterLine);
            }
        }
        else
