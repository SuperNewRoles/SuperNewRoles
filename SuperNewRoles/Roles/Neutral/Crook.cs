...
public static class Crook
{
    ...
    internal static class Ability
    {
        ...
        {
            if (SignDictionary.ContainsKey(ReportDeadBodyPatch.MeetingCount.allTurn))
            {
                SignDictionary[ReportDeadBodyPatch.MeetingCount.allTurn][crookId] = TargetId;
            }
            else
            {
                ...
                SignDictionary.Add(ReportDeadBodyPatch.MeetingCount.allTurn, dic);
            }
        ...
        internal static void SaveReceiptOfInsuranceProceeds()
        {
            ...
            var previousTurn = (byte)(ReportDeadBodyPatch.MeetingCount.allTurn - 1); // 前回ターンのターン数
        ...
        internal static (bool, string) GetIsReceivedTheInsuranceAndAnnounce()
        {
            ...
            byte previousTurn = (byte)(ReportDeadBodyPatch.MeetingCount.allTurn - 1);
        ...
        internal static class InHostMode
        {
            ...
            internal static void TimeoutCountdownAnnounce()
            {
                ...
                foreach (var crook in RoleData.Player)
                {
                    ...
                    // 以下順番前後変更禁止
                    if (!isAllladyCountdown && !SignDictionary.ContainsKey(ReportDeadBodyPatch.MeetingCount.allTurn)) // 今回の会議の保険契約情報が, 誰一人保存されていないなら
                    {
                    ...
                    else if (!isAllladyCountdown && !SignDictionary[ReportDeadBodyPatch.MeetingCount.allTurn].ContainsKey(crook.PlayerId)) // 特定の詐欺師の, 今回の会議の保険契約情報が保存されていないなら
                    {
                        IsAllladyCountdownToSecond[crook.PlayerId] = true;
                        AddChatPatch.ChatInformation(crook, ModTranslation.GetString("CrookName"), warningStr, "#60a1bd");
                    }
            ...
            internal static bool MeetingHudCastVote_Prefix(byte crookId, byte targetId)
            {
                ...
                if (SignDictionary.ContainsKey(ReportDeadBodyPatch.MeetingCount.allTurn) && SignDictionary[ReportDeadBodyPatch.MeetingCount.allTurn].ContainsKey(crookId)) return true; // 既に契約させているなら 時間判定を行わず 有効票を返す。
